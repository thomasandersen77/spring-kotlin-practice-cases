...
| 42 spring-mvc-validation | API / SOLID | Løst | 7.5 | 2026-08-22 | case-42-forsoek-1 |
...
| 53 openai-legal-contract-review | Integrasjon / LLM | Ikke startet | – | – | – |

**Oppsummert:** 5 løst · 3 påbegynt · 45 ikke startet · 0 mestret
...
| 2026-08-22 | 42 | 1 | case-42-forsoek-1 | 7.5 | Løst. Tynn `@RestController`, forretningslogikk holdt i `TaskService`, og `@field:NotBlank`/`@field:Size` brukt korrekt med use-site targets. Controllerkontrakt (`@WebMvcTest` + `@MockBean`) er tydelig skilt fra ren enhetstest av servicen. Alle README-grensene er nå dekket: 201 happy path, 400 for blank og for lang tittel, grensetilfellet 80 tegn gir 201, default-prioritet blir `NORMAL`, og `verifyNoInteractions(service)`/`verify(service)` dokumenterer at ugyldig input ikke delegerer. Modulen er verifisert grønn med `./mvnw test -pl case-42-spring-mvc-validation`. Trekk fordi `TaskService.create` er nær en pass-through med hardkodet `id = 1L` (ingen reell tildelingsflyt), den stabile API-feilmodellen (TODO 5 / debrief-spørsmål 6) ikke er adressert, og mindre testhygiene: wildcard-import `org.mockito.Mockito.*`, en løs tab-innrykket linje og blandet bruk av manuell JSON-streng vs. `ObjectMapper`. Muntlig debrief og tidsbruk er ikke vurdert. |
