 ...
 * REST CONTROLLER
 *
 * Supertynn controller: mapper request DTO -> domain, delegerer til service, returnerer response
 * DTO.
 */
@RestController
...
class OrderController(private val orderService: OrderService) {
	...
	@GetMapping("/{id}")
	fun getOrder(@PathVariable id: UUID): OrderResponse = orderService.getOrder(id).toResponse()
...
data class CreateOrderRequest(
	...
	@field:NotEmpty val lines: List<CreateOrderLineRequest>,
) {
	fun toDomain(): Order =
		Order(
			id = UUID.randomUUID(),
