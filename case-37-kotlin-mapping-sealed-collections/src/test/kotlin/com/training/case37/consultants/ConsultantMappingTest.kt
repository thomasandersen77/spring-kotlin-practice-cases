...
class ConsultantMappingTest {
 ...
 @Test fun `alle statuskoder oversettes til sealed varianter`() {
 ...
 @Test fun `ugyldige rader filtreres og nullable skills blir tom liste`() {
 val inputs = listOf(
 ...
 assertThat(inputs.toDomainConsultants()).containsExactly(Consultant("1", "Kari", 10, Availability.Available, emptyList()))
 }

 @Test fun `mapping beholder bare sertifiserte skills`() {
 val input = ConsultantInput("1", "Kari", 10, "AVAILABLE", null, listOf(kotlin, docker))
 assertThat(input.toDomain()!!.toDto()).isEqualTo(ConsultantDto("1", "Kari", 10, "Tilgjengelig", listOf("Kotlin")))
 }

 @Test fun `kandidater sorteres pa erfaring desc og navn asc ved likhet`() {
 val values = listOf(
 ...
 assertThat(values.rankAvailable().map { it.displayName }).containsExactly("Kotlin", "AWS", "Spring")
 }

 @Test fun `skillrapport teller og sorterer presist`() {
 val values = listOf(
 ...
 assertThat(values.skillPopularity()).containsExactly(SkillPopularityDto("Kotlin", 2), SkillPopularityDto("AWS", 1), SkillPopularityDto("Spring", 1))
 }

 @Test fun `tomme lister gir tomme resultater`() {
