...
fun main(args: Array<String>) {
	runApplication<SecurityCaseApplication>(*args)
}
...
class CaseEntity(
	...
	@Column(nullable = false) var ownerSubject: String = "",
	@Enumerated(EnumType.STRING) @Column(nullable = false) var status: CaseStatus = CaseStatus.OPEN,
)
...
class AuditEntity(
	@Id @GeneratedValue(strategy = GenerationType.IDENTITY) var id: Long? = null,
	@Column(nullable = false) var caseId: Long = 0,
	@Column(nullable = false) var actorSubject: String = "",
	@Column(nullable = false) var correlationId: String = "",
	@Column(nullable = false) var createdAt: Instant = Instant.EPOCH,
)
...
class CorrelationInterceptor : HandlerInterceptor {
	...
	override fun afterCompletion(
		...
		/* TODO: CorrelationContext.clear() */
	}
}

@Configuration
class WebConfig(private val interceptor: CorrelationInterceptor) : WebMvcConfigurer {
...
class CaseService(private val cases: CaseRepository, private val audits: AuditRepository) {
	...
	fun get(id: Long, currentUser: CurrentUser): CaseResponse = TODO("Hent, autoriser og map")

	// TODO 5 og 8: Aktiver @PreAuthorize og gjør status+audit atomisk.
	@Transactional
	fun approve(id: Long, currentUser: CurrentUser): CaseResponse = TODO("Godkjenn og lagre audit")
}
...
class CaseController(private val service: CaseService, private val mapper: CurrentUserMapper) {
	@GetMapping("/{id}")
	fun get(@PathVariable id: Long, @AuthenticationPrincipal jwt: Jwt) =
		service.get(id, mapper.from(jwt))

	@PostMapping("/{id}/approve")
	fun approve(@PathVariable id: Long, @AuthenticationPrincipal jwt: Jwt) =
		service.approve(id, mapper.from(jwt))
}
...
class SecurityCaseErrors {
	...
	fun notFound(ex: CaseNotFound) = mapOf("error" to ex.message)

	@ExceptionHandler(AccessDenied::class)
	@ResponseStatus(HttpStatus.FORBIDDEN)
	fun denied(ex: AccessDenied) = mapOf("error" to ex.message)
}
