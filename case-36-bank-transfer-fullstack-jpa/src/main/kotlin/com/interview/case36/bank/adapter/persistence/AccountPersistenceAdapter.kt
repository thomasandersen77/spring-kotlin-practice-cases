...
/**
 * Adapts [AccountJpaRepository] (Spring Data) to the application's [AccountRepository] port. This is
 * the only class in this case that is allowed to know both the domain model and the JPA entity model.
 */
@Repository
class AccountPersistenceAdapter(
     ...
     *
     * If an entity with this id already exists, look it up first so the existing `@Version` value can
     * be carried over via `toEntity(existingVersion = ...)` - otherwise Hibernate's optimistic lock
     * check would be defeated by always writing version 0. If it does not exist yet, this is a new
