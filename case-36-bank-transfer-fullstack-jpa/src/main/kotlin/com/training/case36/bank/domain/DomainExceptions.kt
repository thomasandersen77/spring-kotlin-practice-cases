...
/**
 * Base type for unchecked, domain-language exceptions. [code] is a stable, machine-readable
 * identifier that [com.training.case36.bank.adapter.web.BankingExceptionHandler] maps to an HTTP
 * status - see TODO 14 there.
 */
...
class AccountBlockedException(accountId: AccountId) :
	BankingException(
 ...
 * the same account, or a non-positive amount. These checks are deliberately cheap: they run before
 * any repository lookup.
 */
class InvalidTransferException(message: String) :
	BankingException(
