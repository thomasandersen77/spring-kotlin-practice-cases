 ...
 * (1/100 krone) so that arithmetic never touches floating point. [Double] is never used for money
 * in this case.
 *
 * Design decision: Money can never be negative. A single non-negative invariant is enough to cover
 * both use cases in this domain - an account balance can never go negative, and a transfer amount
 * can never be negative - so there is no need for two separate money-like types. If this domain
 * ever needed genuinely negative amounts (e.g. a signed ledger entry), that would be a different
 * type.
 *
 ...
 * while the domain and the database only ever see whole øre as a [Long]. The mapping between the
 * two must be explicit and exact.
 */
@JvmInline
value class Money private constructor(val amountOre: Long) : Comparable<Money> {
	 ...
	 *   invariant guard - callers that need to know up front whether there is enough money should
	 *   use [isLessThan] before subtracting (see [BankAccount.debit]).
	 */
	...
	/**
	 * TODO 1: Convert this amount back to kroner with exactly two decimals, e.g. 25_000 øre ->
	 * 250.00, 1 øre -> 0.01. The conversion must be deterministic: the same øre value always
	 * produces the same [BigDecimal], with scale exactly 2, and no rounding is ever needed since
	 * øre is already the smallest unit of NOK.
	 */
	...
	companion object {
		...
		/**
		 * Creates a [Money] directly from a whole number of øre. Used by persistence mapping, where
		 * the database already stores balances and transfer amounts as a `Long` number of øre.
		 *
		...
		/**
		 * TODO 1: Creates a [Money] from a kroner amount received from the API (JSON ->
		 * [BigDecimal]).
		 *
		 * Contract:
		...
		fun ofKroner(amountKroner: BigDecimal): Money =
			TODO(
				"TODO 1: valider ikke-negativt belop og maks to desimaler, konverter eksakt til ore"
			)
	}
}
