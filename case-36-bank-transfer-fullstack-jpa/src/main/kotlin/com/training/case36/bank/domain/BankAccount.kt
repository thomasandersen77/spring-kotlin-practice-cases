 ...
 * whether it is active, whether a withdrawal is allowed, and how its balance changes. No other
 * class
 * - not the application service, not the controller, not the persistence adapter - is allowed to
 *   set [balance] directly. That is why [balance] has a private setter: the only way to change it
 *   is through [credit] and [debit].
 *
 * The constructor is private. Use [open] to create a brand new account, or [reconstitute] to
 * rebuild an existing account from persistence (see `PersistenceMapping.kt`, TODO 5).
 */
class BankAccount
private constructor(
	val id: AccountId,
	...
	companion object {
		...
		/**
		 * Rebuilds an already-existing account, e.g. when mapping an
		 * [com.training.case36.bank.adapter.persistence.AccountJpaEntity] back to the domain.
		 * Unlike [open], this does not force a zero starting balance or ACTIVE status.
		 */
		fun reconstitute(
			id: AccountId,
			ownerName: String,
