 ...
 * -> H2). Account setup for the "blocked account" scenario goes through [accountRepository]
 * directly (there is no HTTP endpoint to block an account in this case) - everything that is
 * actually under test (the transfer request itself) still goes through MockMvc.
 */
@SpringBootTest
...
class BankingControllerIntegrationTest {
	...
	fun `hent konto gir 200 og korrekt DTO`() {
		...
		mockMvc
			.perform(get("/api/accounts/$accountId"))
	...
	fun `ukjent konto gir 404`() {
		mockMvc
			.perform(get("/api/accounts/00000000-0000-0000-0000-000000000099"))
		...
		mockMvc
			.perform(
				post("/api/transfers")
					.contentType(MediaType.APPLICATION_JSON)
					.content(
						"""{"fromAccountId":"$fromId","toAccountId":"$toId","amount":250.00}"""
					)
			)
			.andExpect(status().isOk)
		...
		mockMvc
			.perform(
				post("/api/transfers")
					.contentType(MediaType.APPLICATION_JSON)
					.content(
						"""{"fromAccountId":"$fromId","toAccountId":"$toId","amount":-50.00}"""
					)
			)
		...
		mockMvc
			.perform(
				post("/api/transfers")
					.contentType(MediaType.APPLICATION_JSON)
					.content(
						"""{"fromAccountId":"$accountId","toAccountId":"$accountId","amount":10.00}"""
					)
			)
			.andExpect(status().isBadRequest)
	}
		...
		mockMvc
			.perform(
				post("/api/transfers")
					.contentType(MediaType.APPLICATION_JSON)
					.content(
						"""{"fromAccountId":"$fromId","toAccountId":"$toId","amount":100.00}"""
					)
			)
	...
	fun `blokkert avsenderkonto gir 409`() {
		val blocked =
			TestDataFactory.blockedAccount(ownerName = "Blokkert Bruker", balanceKroner = "1000.00")
		...
		mockMvc
			.perform(
				post("/api/transfers")
					.contentType(MediaType.APPLICATION_JSON)
					.content(
						"""{"fromAccountId":"${blocked.id.value}","toAccountId":"$toId","amount":10.00}"""
					)
			)
		...
		mockMvc
			.perform(
				post("/api/transfers")
					.contentType(MediaType.APPLICATION_JSON)
					.content(
						"""{"fromAccountId":"$fromId","toAccountId":"$toId","amount":300.00}"""
					)
			)
			.andExpect(status().isOk)

		val fromAccount =
			accountRepository.findById(AccountId(UUID.fromString(fromId)))
				?: error("expected sender account to exist")
		val toAccount =
			accountRepository.findById(AccountId(UUID.fromString(toId)))
