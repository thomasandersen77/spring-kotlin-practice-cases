...
class CapacityWireMockTest(@Autowired val adapter: RestClientCapacityAdapter) {
	...
	fun `200 mappes og riktig path og header sendes`() {
		wireMock.stubFor(
			get(urlEqualTo("/external/consultants/C-1/capacity"))
				.willReturn(
					okJson(
						"""{"consultant_ref":"C-1","state_code":"FREE","periods":[],"skills":["Kotlin"]}"""
					)
				)
		)
		assertThat(adapter.getCapacity("C-1").availability).isEqualTo(Availability.Available)
		wireMock.verify(
			getRequestedFor(urlEqualTo("/external/consultants/C-1/capacity"))
				.withHeader(
					"X-Api-Key",
	...
	fun `ekstern 404 blir ikke funnet`() {
		...
		assertThatThrownBy { adapter.getCapacity("404") }
			.isInstanceOf(ConsultantNotFound::class.java)
	}

	@Test
	fun `ekstern 500 blir utilgjengelig leverandor`() {
		wireMock.stubFor(get(anyUrl()).willReturn(serverError()))
		assertThatThrownBy { adapter.getCapacity("500") }
	...
	fun `ugyldig json blir ugyldig leverandorrespons`() {
		...
		assertThatThrownBy { adapter.getCapacity("bad") }
	...
	fun `ukjent statuskode avvises`() {
		wireMock.stubFor(
			get(anyUrl())
				.willReturn(
					okJson(
						"""{"consultant_ref":"C-1","state_code":"ALIEN","periods":[],"skills":[]}"""
					)
				)
		)
		assertThatThrownBy {
				adapter.getCapacity("C-1")
			}
			.isInstanceOf(InvalidProviderResponse::class.java)
	}
	...
	fun `treg leverandor oversettes til utilgjengelig`() {
		wireMock.stubFor(get(anyUrl()).willReturn(okJson("{}").withFixedDelay(1500)))
		assertThatThrownBy {
				adapter.getCapacity("slow")
			}
			.isInstanceOf(ProviderUnavailable::class.java)
	}
}
