package com.training.case51.comparison
...
class FrameworkComparisonTest {
	...
	fun `spring velges pragmatisk ved JPA method security og eksisterende kompetanse`() {
		...
		assertThat(recommendation.rejectedAlternatives)
			.containsKeys(WebFramework.HTTP4K, WebFramework.KTOR)
	}

	@Test
	fun `anbefalingen kan forsvare kotlin native alternativer`() {
		assertThat(recommend(Requirements(false, true, false, false)).framework)
			.isEqualTo(WebFramework.HTTP4K)
		assertThat(recommend(Requirements(false, false, true, false)).framework)
