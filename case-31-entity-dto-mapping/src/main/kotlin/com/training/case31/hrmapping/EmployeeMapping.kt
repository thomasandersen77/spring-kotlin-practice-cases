 ...
 * MAPPING: ENTITET -> DTO
 *
 * Entitetene under er "persistensmodellen": mange felter, noen nullable, noen sensitive. DTO-ene er
 * API-modellen: færre felter, andre navn, og innhold som avhenger av hvem som spør.
 *
 * Alle mapping-funksjonene er uløste (`TODO()`). Testene beskriver kontrakten. Se README for
...
/** TODO 1: Implementer som extension property. Kontrakt: "$firstName $lastName". */
val EmployeeEntity.fullName: String
	get() = TODO("Implementer fullName som extension property på EmployeeEntity")

/**
 * TODO 2: Implementer som extension function med expression body. Kontrakt: ansatt er aktiv når
 * employmentStart <= date OG (employmentEnd == null ELLER employmentEnd >= date).
 */
 ...
 * - `certifiedSkills` = navn på sertifiserte skills, sortert på yearsOfExperience desc, deretter
 *   navn asc
 */
...
/**
 * TODO 5: Map OG filtrer i én operasjon: bare ansatte som er aktive på `today` skal med. Hint:
 * dette er forskjellen på `map`, `filter` + `map`, og `mapNotNull` — vær klar til å begrunne
 * valget.
 */
...
/**
 * TODO 6: Aggregert mapping: grupper ansatte per avdelingsnavn og bygg sammendrag. Kontrakt:
...
fun List<EmployeeEntity>.toDepartmentSummaries(
	today: LocalDate,
	topSkillCount: Int = 3,
): List<DepartmentSummaryDto> = TODO("Grupper per avdeling og bygg DepartmentSummaryDto")
