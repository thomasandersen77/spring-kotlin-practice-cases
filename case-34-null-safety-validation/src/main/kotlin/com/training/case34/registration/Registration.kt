 ...
 *
 * `RegistrationForm` er rådata fra et webskjema: alt er nullable, alt kan være blankt, og datoer er
 * strenger. `Customer` er den validerte domenetypen der bare `phone` og `referralCode` fortsatt kan
 * være null — fordi de faktisk er valgfrie.
 *
 * Validatoren skal samle ALLE feil, ikke stoppe på den første, og aldri kaste exception for
 * forventet ugyldig input. Ingen `!!`.
 *
...
/**
 * TODO 1: Trimmet verdi hvis strengen finnes og ikke er blank, ellers `null`. Hint:
 * `?.trim()?.takeIf { it.isNotEmpty() }` — én expression body, ingen if/else.
 */
fun String?.normalizedOrNull(): String? = TODO("Implementer normalisering av nullable/blank string")

/**
 * TODO 2: Parse ISO-dato (`yyyy-MM-dd`) uten å kaste exception. Returner `null` ved ugyldig input.
 * Hint: `runCatching { }.getOrNull()`.
 */
fun String?.toLocalDateOrNull(): LocalDate? = TODO("Implementer trygg datoparsing")
...
class RegistrationValidator(private val today: LocalDate) {
	...
	/**
	 * TODO 3: Valider skjemaet og bygg `Customer`. Returner `ValidationResult.Valid` med
	 * normaliserte verdier, eller `ValidationResult.Invalid` med ALLE feil (maks én per felt,
	 * sortert på feltnavn asc). Reglene og de eksakte feilmeldingene står i README.
	 */
