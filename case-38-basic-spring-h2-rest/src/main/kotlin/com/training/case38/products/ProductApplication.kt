...
class ProductEntity(
	...
	fun changeStock(delta: Int) {
		TODO("Beskytt stock-invarianten")
	}
}
...
class ProductService(private val repository: ProductRepository) {
	@Transactional
	fun create(request: CreateProductRequest): ProductResponse = TODO("Lagre produkt")

	@Transactional(readOnly = true)
	fun get(id: Long): ProductResponse = TODO("Hent eller kast ProductNotFound")

	@Transactional(readOnly = true)
	fun listActive(): List<ProductResponse> = TODO("Bruk repository-query og map")

	@Transactional
	fun changeStock(id: Long, delta: Int): ProductResponse = TODO("Hent, endre invariant og map")
}
...
class ProductController(private val service: ProductService) {
	...
	fun changeStock(@PathVariable id: Long, @RequestBody request: ChangeStockRequest) =
		service.changeStock(id, request.delta)
}
...
class ProductExceptionHandler {
	...
	fun notFound(ex: ProductNotFound) = mapOf("error" to ex.message)

	@ExceptionHandler(StockConflict::class)
	@ResponseStatus(HttpStatus.CONFLICT)
	fun conflict(ex: StockConflict) = mapOf("error" to ex.message)
}
