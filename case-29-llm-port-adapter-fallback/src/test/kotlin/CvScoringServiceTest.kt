...
class CvScoringServiceTest {
    ...
    private class FakeLlmClient(
        private val behaviour: (ModelId) -> ScoringResult
    ) : LlmClient {
        val calls = mutableListOf<ModelId>()
        override fun scoreCv(model: ModelId, cv: CvDocument, request: ProjectRequest): ScoringResult {
            calls.add(model)
    ...
    fun `hallucinated score outside 0-100 is rejected at construction`() {
        assertThatThrownBy { Score(180) }
            .isInstanceOf(IllegalArgumentException::class.java)
        assertThatThrownBy { Score(-5) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }
}
