...
class DeliveryRoute {
	fun book(
		slot: DeliverySlot,
		packageId: String,
		existing: List<PackageBooking>,
	): PackageBooking {
		TODO(
			"Implement booking invariants: slot capacity, duplicate package handling, and invalid input protection"
		)
	}

	fun sortedByTime(bookings: List<PackageBooking>): List<PackageBooking> {
		TODO(
			"Implement deterministic sorting that uses slot window start time instead of booking insertion order"
		)
	}
}
