...
class CustomerNotFoundException(customerId: CustomerId) : DomainException(
    code = "CUSTOMER_NOT_FOUND",
    message = "Customer ${customerId.value} was not found"
)

class AccountNotFoundException(accountId: AccountId) : DomainException(
...
class AccountNumberAlreadyExistsException(accountNumber: String) : DomainException(
    code = "ACCOUNT_NUMBER_ALREADY_EXISTS",
    message = "Account number $accountNumber already exists"
)

class CustomerHasAccountsException(customerId: CustomerId) : DomainException(
    code = "CUSTOMER_HAS_ACCOUNTS",
    message = "Customer ${customerId.value} still has bank accounts"
)

class AccountHasBalanceException(accountId: AccountId) : DomainException(
    code = "ACCOUNT_HAS_BALANCE",
    message = "Account ${accountId.value} cannot be deleted while balance is not zero"
)

class InsufficientFundsException(accountId: AccountId) : DomainException(
    code = "INSUFFICIENT_FUNDS",
    message = "Account ${accountId.value} has insufficient funds"
)

class AccountClosedException(accountId: AccountId) : DomainException(
    code = "ACCOUNT_CLOSED",
