...
class CustomerAccountRepositoryJpaTest {
	...
	@Autowired lateinit var customerRepository: SpringDataCustomerRepository

	@Autowired lateinit var accountRepository: SpringDataBankAccountRepository

	@Test
	fun `customer can be saved and fetched`() {
		val customer =
			customerRepository.save(CustomerEntity(UUID.randomUUID(), "Ola", "ola@example.no"))
	...
	fun `account can be saved and fetched with owner`() {
		val customer =
			customerRepository.save(CustomerEntity(UUID.randomUUID(), "Ola", "ola2@example.no"))
		val account =
	...
	fun `multiple accounts can belong to same customer`() {
		val customer =
			customerRepository.save(CustomerEntity(UUID.randomUUID(), "Kari", "kari2@example.no"))
		accountRepository.save(
			BankAccountEntity(
				UUID.randomUUID(),
				"15030022222",
	...
	fun `email should be unique`() {
		...
		assertThatThrownBy {
				customerRepository.saveAndFlush(
					CustomerEntity(UUID.randomUUID(), "B", "same@example.no")
				)
			}
	...
	fun `account number should be unique`() {
		val customer =
			customerRepository.save(CustomerEntity(UUID.randomUUID(), "Ola", "ola3@example.no"))
	...
	fun `updated balance should persist`() {
		val customer =
			customerRepository.save(CustomerEntity(UUID.randomUUID(), "Ola", "ola4@example.no"))
		val account =
			accountRepository.saveAndFlush(
				BankAccountEntity(
					UUID.randomUUID(),
	...
	fun `status enum should be stored as string`() {
		val customer =
			customerRepository.save(CustomerEntity(UUID.randomUUID(), "Ola", "ola5@example.no"))
		val account =
			accountRepository.saveAndFlush(
				BankAccountEntity(
					UUID.randomUUID(),
