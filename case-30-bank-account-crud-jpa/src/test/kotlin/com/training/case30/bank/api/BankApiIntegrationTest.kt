...
class BankApiIntegrationTest {
	...
	fun `get unknown customer should return 404`() {
		mockMvc
			.perform(get("/api/customers/00000000-0000-0000-0000-000000000099"))
	...
	fun `deleting account with balance should return 409`() {
		...
		mockMvc.perform(delete("/api/accounts/$accountId")).andExpect(status().isConflict)
	}
	...
	fun `customer without accounts should support full crud flow`() {
		...
		mockMvc.perform(get("/api/customers/$createdCustomerId")).andExpect(status().isOk)
		...
		mockMvc.perform(delete("/api/customers/$createdCustomerId")).andExpect(status().isNoContent)

		mockMvc.perform(get("/api/customers/$createdCustomerId")).andExpect(status().isNotFound)
	}
