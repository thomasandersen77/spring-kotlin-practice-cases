package com.training.case32.notifications
 ...
 *
 * Tre sealed hierarkier: hva som skal varsles (`Notification`), hvor det kan sendes (`Channel`), og
 * hva som skjedde (`DeliveryResult`). Reglene skal uttrykkes som `when`-uttrykk uten `else`, slik
 * at kompilatoren sier fra når en ny variant legges til.
 *
...
/**
 * TODO 1: Implementer som extension property med `when` uten `else`. Kontrakt: Email -> "e-post",
 * Sms -> "SMS", InApp -> "app".
 */
...
object NotificationPolicy {
	...
	/**
	 * TODO 2: Hvilke kanaler skal brukes? `when` over `Notification` uten `else`. Kontrakt
	 * (rekkefølgen i listen er en del av kontrakten):
	 * - PasswordReset -> [Email] hvis mottakeren har e-post, ellers tom liste (ingen InApp:
	 ...
	 * - MarketingCampaign -> [Email] bare hvis mottakeren har både samtykke og e-post, ellers tom
	 *   liste
	 */
	...
	/**
	 * TODO 3: Prioritet som `when`-uttrykk. Kontrakt: PasswordReset og PaymentFailed -> HIGH,
	 * OrderShipped -> NORMAL, MarketingCampaign -> LOW.
	 */
	fun priorityOf(notification: Notification): Priority =
		TODO("Implementer prioritet som when-uttrykk")

	/**
	 * TODO 4: Hvorfor ble ingenting sendt? Kontrakt:
...
/**
 * TODO 6: Oppsummer en batch med resultater. Kontrakt:
 * - tellere per variant
...
/**
 * TODO 7: Sorter varsler slik at de viktigste kommer først, og bevar innbyrdes rekkefølge innenfor
 * samme prioritet (stabil sortering).
 */
